...
export default function Home() {
    ...
    const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (Math.abs(delta) > 40) {
            if (delta > 0) nextMentor(); else prevMentor();
        }
        dragStartX.current = null;
        setTimeout(() => { isDragging.current = false; }, 10);
    };

    // Wheel / trackpad scroll to swipe
    useEffect(() => {
        const el = trackRef.current;
        if (!el) return;
        const onWheel = (e: WheelEvent) => {
            // Use deltaX for horizontal scroll, or deltaY for vertical scroll wheel
        ...
        <>
            ...
            <main className="relative pt-28 sm:pt-40 lg:pt-56 pb-20 sm:pb-32 px-4 sm:px-8 overflow-hidden grid-bg">
                <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
                    ...
                    {/* Mentor Carousel */}
                    <div className="relative h-[440px] sm:h-[520px] lg:h-[600px] flex items-center justify-center order-1 lg:order-2 overflow-visible">
                        {/* Ambient glows */}
